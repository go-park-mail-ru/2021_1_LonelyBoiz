...
func (repo *RepoSqlx) AddUser(newUser model.User) (int, error) {
	var id int
...
func (repo *RepoSqlx) GetUser(id int) (model.User, error) {
	var user []model.User
...
func (repo *RepoSqlx) ChangeUser(newUser model.User) error {
	_, err := repo.DB.Exec(
		`UPDATE users 
...
func (repo *RepoSqlx) SignIn(email string) (model.User, error) {
	var user []model.User
	err := repo.DB.Select(&user, `SELECT * FROM users WHERE email = $1`, email)
	if err != nil {
		return model.User{}, err
