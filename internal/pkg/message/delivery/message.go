...
func (m *MessageHandler) GetMessages(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		model.Process(model.LoggerFunc(st.Message(), m.Usecase.LogError), model.ResponseFunc(w, int(st.Code()), st.Message()), model.MetricFunc(int(st.Code()), r, st.Err()))
		return
	}
	...
	model.Process(model.LoggerFunc("Success: Get Messages", m.Usecase.LogInfo), model.ResponseFunc(w, 200, nMesssages), model.MetricFunc(200, r, nil))
}
...
func (m *MessageHandler) SendMessage(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		model.Process(model.LoggerFunc(err, m.Usecase.LogInfo), model.ResponseFunc(w, 400, response), model.MetricFunc(400, r, response))
		return
	}
	...
	model.Process(model.LoggerFunc("Success Create Message", m.Usecase.LogInfo), model.ResponseFunc(w, 200, nMessage), model.MetricFunc(200, r, nil))
}
...
func (m *MessageHandler) ChangeMessage(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		response := model.ErrorResponse{Err: "Не удалось прочитать тело запроса"}
		model.Process(model.LoggerFunc(response.Err, m.Usecase.LogInfo), model.ResponseFunc(w, 400, response), model.MetricFunc(400, r, response))
		return
	}
	...
	model.Process(model.LoggerFunc("New message", m.Usecase.LogInfo), model.ResponseFunc(w, 204, nMessage), model.MetricFunc(204, r, nil))
}
